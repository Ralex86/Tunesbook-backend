...
function Abc(user) {
    ...
    function error(sev, s, msg, a1, a2, a3, a4) {
        ...
        if (arguments.length > 3) msg = msg.replace(/\$./g, function(a) {
            switch (a) {
                case "$1":
                    return a1;
                case "$2":
        ...
        if (s && s.ctx) errbld(sev, msg, s.ctx.fname, s.istart);
        else errbld(sev, msg)
    }
    ...
    function syntax(sev, msg, a1, a2, a3, a4) {
        ...
        if (arguments.length > 2) msg = msg.replace(/\$./g, function(a) {
            switch (a) {
                case "$1":
                    return a1;
                case "$2":
    ...
    function abc2svg_init() {
        font_init();
        if (!user.anno_start) anno_start = empty_function;
        if (!user.anno_stop) anno_stop = empty_function;
        init_tune()
    }
    var dd_tb = {},
        a_de = [];
    ...
    function d_near(de) {
        ...
        de.y = y;
        if (s.type == NOTE) de.x += s.notes[s.stem >= 0 ? 0 : s.nhd].shhd;
    ...
    function deco_def(nm) {
        if (user_deco && user_deco[nm]) return deco_build(nm, user_deco[nm]);
        if (std_deco[nm]) return deco_build(nm, std_deco[nm]);
    ...
    function draw_all_deco() {
        ...
        while (1) {
            ...
            if (f_staff[dd.func]) set_sscale(-1);
            else set_scale(s);
            st = de.st;
            x = de.x;
            ...
            if (de.has_val) {
                if (dd.func != 2 || stv_g.st < 0) out_deco_val(x, y, f, de.val / stv_g.scale, de.defl);
    ...
    function draw_measnb() {
        ...
        if (bar_num > 1) {
            if (cfmt.measurenb == 0) {
                ...
                if (s.prev && s.prev.type != CLEF) s = s.prev;
                x = s.x - s.wl;
                any_nb = true;
    ...
    function tempo_width(s) {
        ...
        if (s.tempo_ca) w += strw(s.tempo_ca);
    ...
    function write_tempo(s, x, y) {
        ...
        if (s.tempo_notes) {
            for (j in s.tempo_notes) x += draw_notempo(s, x, y, s.tempo_notes[j], sc);
            xy_str(x, y, "=");
            x += strw("= ");
            if (s.tempo_ca) {
    ...
    function draw_partempo(st, top) {
        ...
        var ymin = staff_tb[st].topbar + 12,
            ...
            x = 0;
        for (s = tsfirst; s; s = s.ts_next) {
            if (s.type != TEMPO || s.del) continue;
        ...
        if (some_part) {
            ...
            for (s = some_part; s; s = s.ts_next) {
                ...
                if (cfmt.partsbox) xy_str_b(s.x, 2 - ht - h, s.text);
                else xy_str(s.x, 2 - ht - h, s.text);
                anno_stop(s)
            }
        }
        return dy
        ...
        for (s = s1;; s = s.next) {
            switch (s.type) {
                ...
                case REST:
                    ...
                    if (s.head != FULL) y = ((y + 3 + 12) / 6 | 0) * 6 - 12;
                    s.y = y;
                    break
            }
            if (s == s2) break
    ...
    function draw_bar(s, bot, h) {
        ...
        if (st != 0 && s.ts_prev && s.ts_prev.type != BAR) h = staff_tb[st].topbar * staff_tb[st].staffscale;
    ...
    function draw_rest(s) {
        ...
        if (i >= 6) {
            j = y / 6;
            switch (i) {
                default: if (p_staff.stafflines[j + 1] != "|") xygl(x, y + 6 + staffb, "hl1");
                if (i == 9) {
                    y -= 6;
                    j--
                }
            ...
            if (p_staff.stafflines[j] != "|") xygl(x, y + staffb, "hl1")
        }
        x += 8;
        y += staffb + 3;
        for (i = 0; i < s.dots; i++) {
    ...
    function draw_gracenotes(s) {
        ...
        if (s.p_v.key.k_bagpipe || !cfmt.graceslurs || s.slur_start || !s.next || s.next.type != NOTE) return;
        last = g;
        if (last.stem >= 0) {
            yy = 127;
            for (g = s.extra; g; g = g.next) {
    ...
    function draw_basic_note(x, s, m, y_tb) {
        var i, k, y, p, yy, dotx, doty, old_color = false,
            note = s.notes[m],
        ...
        if (note.color) old_color = set_color(note.color);
        else if (note.map && note.map[2]) old_color = set_color(note.map[2]);
        if (p) {
            if (!psxygl(x_note, y_note, p)) xygl(x_note, y_note, p)
        }
        if (dots) {
    ...
    function prev_scut(s) {
        ...
        if (s.next && s.next.type == KEY) s = s.next;
        if (s.next && s.next.type == METER) return s.next;
        return s
    }
    ...
    function draw_slurs(first, last) {
        ...
        while (1) {
            ...
            if (cont) {
                if (!k.p_v.slur_start) k.p_v.slur_start = 0;
                k.p_v.slur_start <<= 4;
    ...
    function draw_tuplet(s1, lvl) {
        ...
        for (s2 = s1; s2; s2 = s2.next) {
            ...
            if (lvl == 0) {
                if (s2.tp1) draw_tuplet(s2, 1);
                if (s2.te0) break
            } else if (s2.te1) break
        }
        if (!s2) {
        ...
        if (some_slur) {
            draw_slurs(s1, s2);
            if (s1.slur_start || s1.sl1) return;
            for (s3 = s1.next; s3 != s2; s3 = s3.next) {
                if (s3.slur_start || s3.slur_end || s3.sl1 || s3.sl2) return
            }
            if (s2.slur_end || s2.sl2) return
        }
        if (lvl == 0) {
            p = s1.tp0;
            s1.tp0 = 0;
        ...
        if (s1.tf[0] == 1) return;
        dir = s1.tf[3];
        if (!dir) dir = s1.stem > 0 ? SL_ABOVE : SL_BELOW;
        if (s1 == s2) {
            nb_only = true
        } else if (s1.tf[1] == 1) {
            nb_only = true;
        ...
        if (nb_only) {
            ...
            ym = a * xm + b;
            if (s1.tf[2] == 0) out_bnum(xm, ym, p);
            else out_bnum(xm, ym, p + ":" + q);
            if (dir == SL_ABOVE) {
                ym += 10;
        ...
        if (s1.tf[1] != 0) error(2, s1, "'what' value of %%tuplets not yet coded");
        dir = s1.tf[3];
        if (!dir) dir = s1.multi >= 0 ? SL_ABOVE : SL_BELOW;
        if (dir == SL_ABOVE) {
            x1 = s1.x - 4;
            y1 = 24;
            if (s1.st == upstaff) {
        ...
        if (s1.tf[2] == 1) return;
        yy = .5 * (y1 + y2);
        if (s1.tf[2] == 0) out_bnum(xm, yy, p, true);
        else out_bnum(xm, yy, p + ":" + q, true);
        if (dir == SL_ABOVE) {
            yy += 9;
            ...
            switch (s.type) {
                case STAVES:
                    ...
                    for (st = 0; st <= nstaff; st++) {
                        x = xstaff[st];
                        if (x < 0) {
                            if (cur_sy.st_print[st]) xstaff[st] = staves_bar || s.x - s.wl - 2;
                            continue
                        }
                        if (cur_sy.st_print[st]) continue;
                        if (staves_bar) {
                ...
                case BAR:
                    st = s.st;
                    if (s.second || s.invis || xstaff[st] < 0) break;
                    anno_start(s);
    ...
    function draw_symbols(p_voice) {
        ...
        first_note = true;
        for (s = p_voice.sym; s; s = s.next) {
            if (s.invis && s.type != NOTE && s.type != REST && s.type != GRACE) continue;
            x = s.x;
            set_color(s.color);
            switch (s.type) {
                case NOTE:
                ...
                case CLEF:
                    ...
                    if (s.second) break;
                    if (s.invis || staff_tb[st].empty) break;
                    set_color(undefined);
                    set_sscale(st);
                    anno_start(s);
                    y = staff_tb[st].y;
    ...
    function set_pos(k, v) {
        ...
        k = k.slice(0, 3);
        if (k == "ste") k = "stm";
        if (curvoice) curvoice.pos = clone(curvoice.pos);
        set_v_param(k, v, "pos")
    }
    ...
    function set_v_param(k, v, sub) {
        if (curvoice) {
            if (sub) curvoice[sub][k] = posval[v];
            else curvoice[k] = v;
            return
        }
        k = [k + "=", v];
    ...
    function set_format(cmd, param, lock) {
        ...
        switch (cmd) {
            ...
            case "stretchlast":
                ...
                if (cmd == "scale") f /= .75;
                else if (cmd == "pagescale") cmd = "scale";
                cfmt[cmd] = f;
                break;
            case "bstemdown":
            case "breakoneoln":
            ...
            case "staffwidth":
                ...
                if (v < 0) syntax(1, "%%staffwidth too big");
                else cfmt.rightmargin = v;
                break;
            case "textoption":
        ...
        for (bol = 0; bol < eof; bol = parse.eol + 1) {
            ...
            switch (line0) {
                case "X":
                    ...
                    if (info.V) {
                        info_sav.V = {};
                        for (i in info.V) info_sav.V[i] = clone(info.V[i])
                    }
                ...
                case "m":
                    ...
                    if ((!cfmt.sound || cfmt.sound != "play") && cfmt.writefields.indexOf(line0) < 0) break;
                    a = text.match(/(.*?)[= ]+(.*)/);
                    if (!a || !a[2]) {
                        syntax(1, err_bad_val_s, "m:");
                        continue
                    }
                    if (a[1].indexOf("n") >= 0) a[1] = a[1].replace("n", ".[',]?");
                    mac[a[1]] = a[2];
                    break;
                case "s":
    ...
    var gene, staff_tb, nstaff, tsnext, realwidth, insert_meter, beta_last, space_tb = new Float32Array([7, 10, 14.15, 20, 28.3, 40, 56.6, 80, 113, 150]),
        smallest_duration;
    const dx_tb = new Float32Array([10, 10, 11, 13, 13]);
    const hw_tb = new Float32Array([4.5, 5, 6, 7, 8]);
    ...
    function unlksym(s) {
        ...
        if (s.ts_next) {
            if (s.seqst) {
                ...
                if (s.ts_next.shrink < s.shrink) s.ts_next.shrink = s.shrink;
                if (s.ts_next.space < s.space) s.ts_next.space = s.space
            }
    ...
    function may_combine(s) {
        ...
        if (s.type == REST) {
            if (s.type == s2.type && s.invis && !s2.invis) return false;
            return true
        }
    ...
    function do_combine(s) {
        ...
        while (1) {
            ...
            if (s.type != s2.type) {
                ...
                if (s.invis && !s2.invis) delete s.invis
            } else {
                combine_notes(s, s2)
            }
            if (s2.text) s.text = s2.text;
            if (s2.a_gch) s.a_gch = s2.a_gch;
            unlksym(s2);
    ...
    function set_graceoffs(s) {
        var next, m, gspleft = Number(cfmt.gracespace[0]),
        ...
        switch (s.type) {
            ...
            case BAR:
                ...
                if (!s.invis) {
                    ...
                    if (s.prev && s.prev.type == GRACE) s.wl -= 8
                } else {
                    s.wl = s.wr = 0
                }
    ...
    function set_space(s) {
        ...
        if (s.ts_prev.type == MREST) return 71;
    ...
    function set_allsymwidth(last_s) {
        ...
        while (1) {
            ...
            do {
                set_width(s);
                if (xl[s.st]) new_val = xl[s.st] + s.wl;
                else new_val = s.wl;
                if (new_val > maxx) maxx = new_val;
                s = s.ts_next
            } while (s != last_s && !s.seqst);
            s2.shrink = maxx - xa;
            if (s2.ts_prev) s2.space = set_space(s2);
            else s2.space = 0;
            ...
            s = s2;
            do {
                if (!xl[s.st] || xl[s.st] < xa + s.wr) xl[s.st] = xa + s.wr;
                s = s.ts_next
            } while (!s.seqst)
        }
    }
    ...
    function set_nl(s) {
        ...
        for (;; s = s.ts_next) {
            ...
            switch (s.type) {
                case STAVES:
                    while (s.ts_next && s.ts_next.type == CLEF) s = s.ts_next;
                    if (!s.ts_next || s.ts_next.type != BAR) continue;
                    s = s.ts_next;
                case BAR:
                    if (done) break;
                    done = 1;
                ...
                case KEY:
                    ...
                    continue;
                default:
                    if (!done || s.prev && s.prev.type == GRACE) continue;
                    break
            }
            break
        }
    ...
    function set_lines(s, last, lwidth, indent) {
        ...
        while (1) {
            ...
            if (!cut_here) {
                ...
                xmax -= 6;
                for (; s != last; s = s.ts_next) {
                    if (s.beam_st) beam = true;
                    if (s.beam_end) beam = false;
                    x = s.x;
                    if (!x || x < xmin) continue;
                    if (x + s.shrink >= xmax) break;
    ...
    function set_auto_clef(st, s_start, clef_type_start) {
        ...
        for (s = s_start; s; s = s.ts_next) {
            if (s.type == STAVES && s != s_start) break;
            if (s.st != st) continue;
            if (s.type != NOTE) {
                if (s.type == CLEF) {
    ...
    function set_clefs() {
        var s, s2, st, v, p_voice, g, new_type, new_line, p_staff, pit, staff_clef = new Array(nstaff),
            sy = cur_sy;
    ...
    function set_pitch(last_s) {
        var s, s2, g, st, delta, m, pitch, note, dur = BASE_LEN,
    ...
    function set_stem_dir() {
        ...
        while (s) {
            ...
            for (u = s; u; u = u.ts_next) {
                ...
                if (u.type != NOTE && u.type != REST || u.invis) continue;
                st = u.st;
                if (st > nst) {
            ...
            while (s && s.type == BAR) s = s.ts_next
        }
    }
    ...
    function init_music_line() {
        ...
        for (v = 0; v < nv; v++) {
            if (cur_sy.voices[v].range < 0 || cur_sy.voices[v].second || !cur_sy.st_print[cur_sy.voices[v].st]) continue;
            p_voice = voice_tb[v];
        ...
        if (insert_meter & 1) {
            for (v = 0; v < nv; v++) {
                p_voice = voice_tb[v];
                s2 = p_voice.meter;
                if (cur_sy.voices[v].range < 0 || cur_sy.voices[v].second || !cur_sy.st_print[cur_sy.voices[v].st] || s2.a_meter.length == 0) continue;
        ...
        for (v = 0; v < nv; v++) {
            ...
            if (cur_sy.voices[v].range < 0 || !cur_sy.st_print[cur_sy.voices[v].st]) continue;
            ...
            if (s2.invis) s.invis = true;
            if (s2.norepbra) s.norepbra = true;
            s.text = s2.text;
            s.a_gch = s2.a_gch;
            if (s2.rbstart) s.rbstart = s2.rbstart
        }
        set_pitch(last_s);
    ...
    function set_words(p_voice) {
        var s, s2, nflags, lastnote, start_flag = true,
            pitch = 127;
        for (s = p_voice.sym; s; s = s.next) {
            if (s.type == NOTE) {
                pitch = s.notes[0].pit;
        ...
        for (s = p_voice.sym; s; s = s.next) {
            switch (s.type) {
                ...
                case BAR:
                    ...
                    if (!s.next && s.prev && s.prev.head == OVALBARS) s.prev.head = SQUARE;
                    break;
                case NOTE:
                case REST:
                    if (s.trem2) break;
    ...
    function set_beams(sym) {
        var s, t, g, beam, s_opp, dy, avg, n, laststem = -1,
            lasty = 0;
        for (s = sym; s; s = s.next) {
            if (s.type != NOTE) {
    ...
    function same_head(s1, s2) {
        ...
        if (head == 0) head = s1.p_v.scale < s2.p_v.scale ? 2 : 1;
        if (head == 1) {
    ...
    function set_overlap() {
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            if (s.type != NOTE || s.invis) continue;
            ...
            while (1) {
                ...
                if (s2.type == NOTE && !s2.invis && s2.st == s.st) break
            }
            if (!s2) continue;
            s1 = s;
    ...
    function set_stems() {
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            if ((scale = s.p_v.scale) != 1) slen *= (scale + 1) * .5;
    ...
    function set_piece() {
        var s, p_voice, st, v, nst, nv, non_empty = [],
            non_empty_gl = [],
            sy = cur_sy;
            ...
            switch (s.type) {
                ...
                case MREST:
                    if (cfmt.staffnonote > 1) {
                        ...
                        if (cfmt.staffnonote != 0 || s.type == NOTE) non_empty_gl[st] = non_empty[st] = true
                    }
                    break
            }
        }
    ...
    function set_sym_glue(width) {
        ...
        if (some_grace) {
            for (s = tsfirst; s; s = s.ts_next) {
                if (s.type != GRACE) continue;
                if (s.gr_shift) x = s.prev.x + s.prev.wr + Number(cfmt.gracespace[0]);
                else x = s.x - s.wl + Number(cfmt.gracespace[0]);
    ...
    function set_kv_parm(a) {
        ...
        while (1) {
            ...
            switch (item) {
                ...
                case "stafflines=":
                    ...
                    if (item == undefined) syntax(1, "Bad stafflines= value");
                    else curvoice.stafflines = item;
                    break;
                default:
        ...
        if (pos) {
            ...
            for (item in pos) curvoice.pos[item] = pos[item]
        }
        if (s) {
            s = new_clef(s);
            if (s) get_clef(s)
    ...
    function new_key(param) {
        var i, clef, key_end, c, tmp, mode = 0,
            s = {
                type: KEY,
                ctx: parse.ctx,
                istart: parse.istart,
    ...
    function new_meter(text) {
        ...
        if (parse.state != 3) {
            ...
            if (parse.state >= 1) {
                if (!glovar.ulen) {
                    if (wmeasure <= 1 || wmeasure >= BASE_LEN * 3 / 4) glovar.ulen = BASE_LEN / 8;
    ...
    function do_info(info_type, text) {
        ...
        switch (info_type) {
            ...
            case "P":
                ...
                if (parse.state == 2) goto_tune();
                if (cfmt.writefields.indexOf(info_type) < 0) break;
                s = {
                    type: PART,
                    ctx: parse.ctx,
                    istart: parse.istart,
    ...
    function new_bar() {
        ...
        if (s2 && s2.type == KEY && (!s2.prev || s2.prev.type != BAR)) {
            ...
            if (!s2.prev) curvoice.sym = s2.prev;
            sym_link(s);
            s.next = s2;
            s2.prev = s;
    ...
    function parse_staves(p) {
        ...
        if (err || a_vf.length == 0) return null;
        return a_vf
    }
    ...
    function identify_note(s, dur) {
        ...
        if (flags >= 0) {
            ...
            default: syntax(1, "Note too long");flags = -4;
            case -4:
                    head = SQUARE;
                break;
            case -3:
    ...
    function parse_acc_pit(line) {
        ...
        if (acc && (c >= "1" && c <= "9") || c == "/") {
            ...
            micro_n = nd[0];
            micro_d = nd[1];
            if (micro_d == 1) micro_d = curvoice.uscale;
            else micro_d *= 2;
            c = line.char()
        }
    ...
    function note_transp(s) {
        var i, j, n, d, a, acc, i1, i3, i4, note, m = s.nhd,
    ...
    function new_note(grace, tp_fact) {
        var note, s, in_chord, c, dcn, type, i, n, s2, nd, res, num, dur, sl1 = 0,
            line = parse.line,
            a_dcn_sav = a_dcn;
        a_dcn = null;
        ...
        switch (c) {
            ...
            default:
                ...
                while (1) {
                    ...
                    if (curvoice.ottava) note.pit += curvoice.ottava;
                    if (sl1) {
                        note.sl1 = sl1;
                        if (s.sl1) s.sl1++;
                        else s.sl1 = 1;
        ...
        sym_link(s);
        if (s.type == NOTE && curvoice.vtransp) note_transp(s);
        if (cfmt.shiftunison) s.shiftunison = cfmt.shiftunison;
        if (!curvoice.lyric_restart && !s.grace) curvoice.lyric_restart = s;
    ...
    function parse_music_line() {
        var s, grace, sappo, dcn, i, c, idx, type, n, text, last_note_sav, a_dcn_sav, no_eol, tp_a = [],
            tp, tpn = -1,
            tp_fact = 1,
            slur_start = 0,
            line = parse.line;
        ...
        for (k in mac) {
            if (k.indexOf(".") >= 0) line.buffer = expand(line.buffer, k);
            else line.buffer = line.buffer.replace(k, mac[k])
        }
        while (1) {
            c = line.char();
            if (!c || c == "%") break;
            ...
            switch (type[0]) {
                ...
                case "-":
                    ...
                    s = curvoice.last_note;
                    for (i = 0; i <= s.nhd; i++) {
                        if (!s.notes[i].ti1) s.notes[i].ti1 = tie_pos;
                        else if (s.nhd == 0) syntax(1, "Too many ties")
                    }
                    s.ti1 = true;
    ...
    function defs_add(text) {
        ...
        while (1) {
            is = text.indexOf("<", ie);
            if (is < 0) break;
            if (text.slice(is, is + 4) == "\x3c!--") {
                ie = text.indexOf("--\x3e", is + 4);
                if (ie < 0) break;
                continue
            }
    ...
    function set_g() {
        ...
        output.push("<g ");
        if (stv_g.scale != 1) {
            if (stv_g.st >= 0) output.push(staff_tb[stv_g.st].scale_str);
    ...
    function set_color(color) {
        if (color == stv_g.color) return null;
    ...
    function g_open(x, y, rot, sx, sy) {
        ...
        if (rot) output.push(") rotate(" + rot.toFixed(2));
        if (sx) {
            if (sy) output.push(") scale(" + sx.toFixed(2) + ", " + sy.toFixed(2));
            else output.push(") scale(" + sx.toFixed(2))
        }
        output.push(')">\n');
        stv_g.g++
    }
    ...
    function svg_flush() {
        var head;
        if (multicol || output.length == 0 || !user.img_out || posy == 0) return;
        posy *= cfmt.scale;
        if (user.imagesize) {
        ...
        if (style || font_style || musicfont) {
            ...
            if (musicfont) head += '@font-face {\n  font-family: "music";\n  src: ' + musicfont + "}\n";
            head += "</style>\n"
        }
        if (defs) head += "<defs>" + defs + "\n</defs>\n";
        if (cfmt.bgcolor) head += '<rect width="100%" height="100%" fill="' + cfmt.bgcolor + '"/>\n';
        if (cfmt.scale == 1) head += '<g class="music" stroke-width=".7">\n';
        else head += '<g class="music" stroke-width=".7" transform="scale(' + cfmt.scale.toFixed(2) + ')">\n';
    ...
    function voice_filter() {
        ...
        for (opt in parse.voice_opts) {
            ...
            if (sel.test(curvoice.id) || sel.test(curvoice.nm)) {
                for (i in parse.voice_opts[opt]) do_pscom(parse.voice_opts[opt][i])
            }
        }
    }
    ...
    const w_tb = new Uint8Array([2, 1, 8, 0, 3, 5, 6, 9, 9, 0, 9, 3, 0, 7, 0, 0, 0, 0]);

    function sort_all() {
        var s, s2, p_voice, v, time, w, wmin, ir, multi, prev, new_sy, nb, nv = voice_tb.length,
            vtb = [],
            vn = [],
            mrest_time = -1;
        for (v in voice_tb) vtb.push(voice_tb[v].sym);
        var fl = 1,
            sy = cur_sy,
            set_sy = true;
        while (1) {
    ...
    function voice_adj() {
        ...
        for (v in voice_tb) {
            ...
            for (; s; s = s.next) {
                switch (s.type) {
                    case GRACE:
                        if (s.next && s.next.type == BAR) s.time--;
        ...
        function do_break() {
            ...
            for (i in glovar.break) {
                ...
                for (s = voice_tb[v].sym; s; s = s.next) {
                    if (s.type == BAR && s.bar_num == m) break
                }
                if (!s) continue;
                if (n != 0) {
                    t = s.time + n / d;
    ...
    function get_midi(param) {
        ...
        switch (a[0]) {
            case "program":
                ...
                if (curvoice) curvoice.instr = prog;
                else glovar.instr = prog;
                break
        }
    }
    ...
    function do_pscom(text) {
        ...
        switch (cmd) {
            ...
            case "setbarnb":
                val = parseInt(param);
                if (isNaN(val)) syntax(1, "Bad %%setbarnb value");
                else if (parse.state >= 2) glovar.new_nbar = val;
            ...
            case "stafflines":
                ...
                else set_v_param(cmd, val);
                return;
            case "staffscale":
                val = parseFloat(param);
                if (isNaN(val) || val < .3 || val > 2) syntax(1, "Bad %%staffscale value");
                else set_v_param(cmd, val);
                return;
            case "staves":
            case "score":
    ...
    function do_begin_end(type, opt, text) {
        var i, j, action, s;
        switch (type) {
            default: if (typeof wpsobj == "object") {
                wpsobj.parse(text);
                output.push(svgbuf)
            }break;
            case "js":
    ...
    function get_staves(cmd, parm) {
        ...
        if (no_sym || maxtime == 0 && staves_found < 0) {
            for (v in par_sy.voices) par_sy.voices[v].range = -1
        } else {
        ...
        if (parse.state >= 2) curvoice = voice_tb[par_sy.top_voice];
        else curvoice = null
    }
    ...
    function get_vover(type) {
        ...
        if (type == "|" || type == ")") {
            ...
            if (curvoice.time != vover.mxtime) syntax(1, "Wrong duration in voice overlay");
    ...
    function get_key(parm) {
        var v, p_voice, s, transp, a = new_key(parm),
            s_key = a[0];
        a = a[1];
        if (s_key.k_sf && !s_key.k_exp && s_key.k_a_acc) set_k_acc(s_key);
        switch (parse.state) {
            case 1:
        ...
        if (a.length != 0) set_kv_parm(a);
        transp = (cfmt.transp || 0) + (curvoice.transp || 0) + (curvoice.shift || 0);
        if (s_key.k_sf == undefined) {
            if (!s_key.k_a_acc && !transp) return;
    ...
    function get_voice(parm) {
        var v, transp, vtransp, a = info_split(parm, 1),
            vid = a.shift();
        if (parse.state < 2) {
            if (a.length != 0) memo_kv_parm(vid, a);
            if (parse.state == 1) new_voice(vid);
            return
        }
    ...
    function draw_all_lyrics() {
        ...
        for (v in voice_tb) {
            ...
            nly_tb[v] = nly;
            if (nly == 0) continue;
            if (p_voice.pos.voc) above_tb[v] = p_voice.pos.voc == SL_ABOVE;
            else if (voice_tb[v + 1] && voice_tb[v + 1].st == st && voice_tb[v + 1].have_ly) above_tb[v] = true;
        ...
        for (v in voice_tb) {
            ...
            st = p_voice.st;
            set_dscale(st, true);
            if (nly_tb[v] > 0) lyst_tb[st].bot = draw_lyrics(p_voice, nly_tb[v], h_tb[v], lyst_tb[st].bot, 1)
        }
        while (--i >= 0) {
            v = rv_tb[i];
            p_voice = voice_tb[v];
    ...
    function gch_transp(s) {
        ...
        function gch_tr1(p) {
            ...
            if (latin >= 0) {
                ...
                i4 = cgd2cde[(i3 + 16 * 7) % 7];
                i1 = (((i3 + 22) / 7 | 0) + 159) % 5;
                if (latin == 0) new_txt = note_names[i4] + acc_name[i1];
                else new_txt = latin_names[i4] + acc_name[i1]
            } else {
                new_txt = ""
            }
            ip2 = p.indexOf("/", ip);
    ...
    function gch_build(s) {
        var gch, w, xspc, ix, pos = curvoice.pos.gch == SL_BELOW ? -1 : 1,
    ...
    function draw_gchord(s, gchy_min, gchy_max) {
        ...
        for (ix in s.a_gch) {
            ...
            switch (gch.type) {
                ...
                default:
                    ...
                    if (i >= 0) {
                        ...
                        if (user.anno_start) user.anno_start("gchord", gch.istart, gch.iend, x - 2, y + h + 2, w + 4, h + 4, s);
                        i = 0;
                        j = i;
                        for (;;) {
                            i = text.indexOf("\t", j);
                        ...
                        if (user.anno_stop) user.anno_stop("gchord", gch.istart, gch.iend, s.x - 2, y + h + 2, w + 4, h + 4, s);
                        continue
                    }
                    break;
                case "@":
            ...
            y *= staff_tb[s.st].staffscale;
            if (user.anno_start) user.anno_start("annot", gch.istart, gch.iend, x - 2, y + h + 2, w + 4, h + 4, s);
            if (gch.box) xy_str_b(x, y + h * .2, text);
            else xy_str(x, y + h * .2, text);
            if (user.anno_stop) user.anno_stop("annot", gch.istart, gch.iend, x - 2, y + h + 2, w + 4, h + 4, s)
        }
    }
